...
- **Quiz doors**: until someone in the quiz room is sharing a screen (the Let's Quiz host app, or the quizmaster from the full site), the quiz page shows a coming-soon notice instead of the join screen, and opens by itself when the share starts. It watches the room's presence without joining. The wording is `soonTitle` / `soonText` in `quiz/index.html`; `doors: false` there turns it off. Players already in stay in if the share drops.
